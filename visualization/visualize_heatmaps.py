...
def grid_visualization(hm_probs: np.ndarray, save_path: str | None, sigma: float = 1.2):
    """hm_probs: (J, Hm, Wm) in [0,1] - Display all 15 joint heatmaps combined in a SINGLE image"""
    from scipy.ndimage import gaussian_filter
    
    J, Hm, Wm = hm_probs.shape
    
    # Combine all 15 joint heatmaps into a single image by taking the maximum across joints
    # This creates one image showing all joint locations as bright spots
    combined_heatmap = np.max(hm_probs, axis=0)  # (Hm, Wm) - max across all 15 joints
    
    # Apply Gaussian blur with sigma=1.2 to smooth the heatmap
    combined_heatmap = gaussian_filter(combined_heatmap, sigma=sigma)
    
    # Create a single figure with the combined heatmap
    fig, ax = plt.subplots(1, 1, figsize=(8, 8))
    fig.suptitle("2D Joint Heatmap - All 15 Joints", fontsize=16, fontweight='bold', color='white')
    
    # Use 'hot' colormap: black -> red -> yellow -> white
    # This creates black background with bright yellowish-white blobs for each joint
    try:
    ...
    # Display the combined heatmap with all joints visible as bright spots
    im = ax.imshow(combined_heatmap, cmap=cmap, interpolation="nearest", vmin=0.0, vmax=1.0)
    ax.axis("off")
    ...
    # Add colorbar showing the scale from 0.0 (black) to 1.0 (yellowish-white)
    cbar = plt.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    cbar.set_label('Intensity', rotation=270, labelpad=15, fontsize=10, color='white')
    ...
    if save_path:
        ...
        print(f"Saved combined heatmap visualization (all 15 joints) to {save_path}")
    plt.show()
...
def main():
    ...
    ap.add_argument("--alpha", type=float, default=0.15, help="Overlay alpha for heatmaps")
    ap.add_argument("--point_size", type=int, default=5, help="Size of joint points")
    args = ap.parse_args()
    ...
    # Grid of probabilities (with Gaussian smoothing sigma=1.2)
    grid_out = os.path.join(args.save_dir, "pred_heatmaps_grid.png") if args.save_dir else None
    grid_visualization(probs[0].cpu().numpy(), grid_out, sigma=1.2)
