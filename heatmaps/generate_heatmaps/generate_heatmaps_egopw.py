...
def process_sequence(seq_dir, expect_joints, hm_size, sigma, overwrite=False, img_subdir="imgs", out_dirname=None):
...
def main():
    parser = argparse.ArgumentParser(description="Generate (15,H,W) heatmaps and save to '<seq>/heatmap{H}_{sigma}/heatmap_XXXXXX.npy'.")
    parser.add_argument("--train_txt", type=str, default='test_egopw.txt', help="Path to train/test list (one sequence directory per line)")
    parser.add_argument("--hm_size", type=int, default=64, help="Heatmap size (square). Default: 64")
    parser.add_argument("--sigma", type=float, default=2.0, help="Gaussian sigma (in heatmap coords). Default: 2.0")
    ...
    parser.add_argument("--out_dirname", type=str, default=None, help="Override output subfolder name (default: heatmap{hm_size}_{sigma})")
    ...
    for seq in seq_dirs:
        ...
        total_saved += saved
        total_items += items
        out_dirname = args.out_dirname or _default_out_dirname(args.hm_size, args.sigma)
        print(f"[SEQ] {seq}: saved {saved}/{items} heatmaps -> {os.path.join(seq, out_dirname)}")
