...
class MaskedMHCA(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        # self-attention: (B, nh, T', hs) x (B, nh, hs, T'') -> (B, nh, T', T'')
        att = (q * self.scale) @ k.transpose(-2, -1)
        kv_resized = kv_mask[:, None, :, :].float()  # Add extra dimension and convert to float
        # kv_resized = kv_resized.unsqueeze(-1)
        kv_mask_resized = F.interpolate(kv_resized, size=(att.shape[2], att.shape[3]), mode='nearest')  # Use only 2 spatial dimensions


        # Expand to match the number of heads in the attention tensor
        kv_mask_resized = kv_mask_resized.expand(-1, att.shape[1], -1, -1)
        # Apply the masked fill operation
        att = att.masked_fill(torch.logical_not(kv_mask_resized.bool()), 0.0)
        # prevent q from attending to invalid tokens
        ...
        # Expand to match the number of heads in the attention tensor
        kv_mask_resized = kv_mask_resized.expand(-1, att.shape[1], -1, -1)
        kv_mask_resized_simplified = kv_mask_resized[:, :, :, 0]  # Shape: [16, 4, 8]
        # Add an extra dimension to broadcast along the feature dimension of v
        v_masked = v * kv_mask_resized_simplified.unsqueeze(-1).to(v.dtype)
        out = torch.matmul(att, v_masked)
        # Perform the matrix multiplication with the transposed v
...
class LocalMaskedMHCA(nn.Module):
    ...
    def __init__(
        ...
        self.window_overlap  = self.window_size // 2
    ...
    def forward(self, x, mask):
        ...
        # Downsample float_inverse_kv_mask to 128 elements using interpolation
        float_inverse_kv_mask = F.interpolate(float_inverse_kv_mask.permute(0, 2, 1), size=att.shape[1], mode='nearest').permute(
            0, 2, 1)
        ...
        # Convert kv_mask to float
        kv_resized = kv_mask[:, None, :, :].float()  # Add extra dimension and convert to float
        kv_mask_resized = F.interpolate(kv_resized, size=(att.shape[2], att.shape[3]), mode='nearest')
